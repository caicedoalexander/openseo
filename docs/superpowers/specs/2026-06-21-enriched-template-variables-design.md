...
`%tag%`, `%parent_title%`— manteniendo paridad completa con el preview SERP en
vivo del editor. Inspirado en Rank Math, pero recortado a lo que aporta valor
...
| `%category%` | Nombre de la **primera** categoría (orden de `get_the_category`) | primera de `get_the_category($id)` → `->name`, `''` si no hay |
| `%tag%` | Nombre de la **primera** etiqueta | primera de `get_the_tags($id)` → `->name`, `''` si no hay (devuelve `false` si no hay tags) |
...
- `%date%`/`%modified%` → `getEditedPostAttribute('date'|'modified')` (ISO),
  ...
  → paridad con el `date_format` del servidor sin tocar el bootstrap.
- `%author%` → `getEditedPostAttribute('author')` (ID) → `core` store
  `getUser(id)` → `name` (display name). `''` mientras resuelve.
...
- **JS (`npm run test:js`):**
  - `preview.test.js` ya cubre `expandTokens`; se añaden casos para los nuevos
    tokens si se extrae algún helper puro (p. ej. selección de "primer término").
  - El hook `useTemplateTokens` (depende de core-data) se valida vía `build` +
    smoke test manual; no se fuerza un test de integración de React.
...
- **`get_the_tags` devuelve `false`** (no `[]`) cuando no hay etiquetas: el
  factory debe normalizar antes de indexar.
